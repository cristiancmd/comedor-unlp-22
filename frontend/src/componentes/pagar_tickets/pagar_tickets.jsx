...
const Pagar_tickets = ({mis_tickets, set_mis_tickets}) => {
    ...
    const [error_numero_de_tarjeta, set_error_numero_de_tarjeta] = useState(false);
    const [error_codigo_de_seguridad, set_error_codigo_de_seguridad] = useState(false);
    const [error_codigo_de_seguridad_vacio, set_error_codigo_de_seguridad_vacio] = useState(false);
    ...
    const borrar = () => {
      let tickets = mis_tickets.filter(ticket => (ticket.menu !== menu_a_borrar || ticket.date !== fecha_a_borrar || ticket.campus !== sede_a_borrar))
    ...
    const pagar = async p => {
      ...
      if (no_hay_errores) {
        await axios.post(url_tickets,mis_tickets).then(response=>{
    ...
    return (
        <>
          ...
          <main>
            <div>
              <Breadcrumb tag="nav" listTag="div">
                ...
                <BreadcrumbItem active tag="span">Pagar tickets</BreadcrumbItem>
              </Breadcrumb>
            </div>
            ...
            <div className="col-10 offset-1">
              ...
              <div className="d-flex justify-content-between">
                <div>
                  <h5 className="text-center">Número de la tarjeta (XXXX-XXXX-XXXX-XXXX)</h5>
                  <div className="d-flex justify-content-center">
                    <input type="text" className="form-control text-center" id="numero_de_tarjeta_pagar_ticket" placeholder="Ingrese su número de tarjeta" onChange={capturar_el_ingreso_de_numero}/>
                  </div>
                  {error_numero_de_tarjeta?<h5 className="text-danger text-center">Ingrese su número de tarjeta con el formato pedido</h5>:""}
                </div>
                <div>
                  <h5 className="text-center">Código de seguridad</h5>
                  <div className="d-flex justify-content-center">
                    <input type="text" className="form-control text-center" id="codigo_de_seguridad_pagar_ticket" placeholder="Ingrese su código" onChange={capturar_el_ingreso_de_codigo}/>
                  </div>
                  {error_codigo_de_seguridad?<h5 className="text-danger text-center">Código inválido</h5>:""}
                  {error_codigo_de_seguridad_vacio?<h5 className="text-danger text-center">Código vacío</h5>:""}
                </div>
                <div>
                  <h5 className="text-center">Fecha de vencimiento</h5>
                  <div className="d-flex justify-content-center">
                    <input type="date" className="form-control text-center text-secondary" id="fecha_de_vencimiento_pagar_ticket" onChange={capturar_el_ingreso_de_fecha}/>
                  </div>
                  {error_fecha?<h5 className="text-danger text-center">Complete los tres campos de la fecha (día, mes y año)</h5>:""}
                </div>
                <div>
                  <h5 className="text-center">Titular de la cuenta (Apellido Nombre)</h5>
                  <div className="d-flex justify-content-center">
                    <input type="text" className="form-control text-center" id="titular_pagar_ticket" placeholder="Ingrese su apellido y nombre" onChange={capturar_el_ingreso_de_titular}/>
                  </div>
                  {error_titular?<h5 className="text-danger text-center">Ingrese su apellido y nombre con el formato pedido</h5>:""}
                </div>
              </div>
            </div>
            ...
            <div className="row mt-5">
              <div className="col-10 offset-1">
                <table className="table">
                  ...
                  <tbody>
                    {mis_tickets.length === 0 && <tr><td colSpan="12"><h6>No ha elegido ningún ticket.</h6></td></tr>}
                    {mis_tickets.map(ticket => {
                      return (
                        <tr>
                          ...
                          <td>
                            <button className="btn btn-primary-outline" onClick={()=>{guardar_datos_temporalmente(ticket.menu,ticket.date,ticket.campus);set_modal_eliminar(true)}}>
            ...
            {mis_tickets.length === 0?
              <div className="d-flex justify-content-center mt-3">
                <button className="btn btn-secondary fs-5" onClick={cancelar_compra}>Cancelar</button>
              </div>:
              <div>
                <h4 className="text-center mt-3 mb-5">Total ${total()}</h4>

                <div className="d-flex justify-content-center">
                  <button className="btn btn-secondary me-3 fs-5" onClick={cancelar_compra}>Cancelar</button>
                  <button className="btn btn-primary ms-3 fs-5" onClick={pagar}>Pagar</button>
                </div>
              </div>       
            }
          </main>
          ...
          <Modal isOpen={modal_eliminar}>
            <ModalBody>
              ¿Estás seguro que querés eliminar el menú: {nombre_del_menu(menu_a_borrar)}?
            </ModalBody>
            <ModalFooter>
              <button className="btn btn-secondary" onClick={()=>set_modal_eliminar(false)}>No</button>
              <button className="btn btn-primary" onClick={()=>{borrar();set_modal_eliminar(false)}}>Sí</button>
            </ModalFooter>
          </Modal>
            ...
            <ModalBody>
              <div className="mt-4 row justify-content-center">
                <div className="col text-center">
                  <FontAwesomeIcon icon={faCheckCircle} color={"#6BC04B"} className="fa-3x"/>
                </div>
              </div>
